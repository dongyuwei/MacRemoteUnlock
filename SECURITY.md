...
| 认证 / Authentication | token（至少 6 位，数字+字母，区分大小写） | `/status` `/approve` `/deny` 必须携带，错误或缺失返回 `401` / required on protected endpoints, wrong/missing gets `401` |
| 暴力防护 / Brute-force protection | 失败限速 | 连续 5 次失败锁定 60 秒（`429`），每次失败响应延迟 1 秒 → 穷举 token 空间需数月到数年 / 5 consecutive failures → 60s lockout (`429`), each failure delayed 1s → brute-forcing the token space takes months to years |
...
- 缓解：默认关闭；token（≥6 位）；失败限速；建议定期轮换 token / mitigations: off by default, token (≥6 chars), rate limiting, rotate the token regularly
...
- **token 空间取决于长度**：至少 6 位（字母+数字），建议用更长的随机 token；单靠短 token 不够强，必须配合网络隔离与限速 / token strength depends on length: minimum 6 chars (letters+digits), longer random tokens recommended; never rely on a short token alone — network isolation and rate limiting are required
